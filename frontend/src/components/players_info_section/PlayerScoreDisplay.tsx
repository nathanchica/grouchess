...
function PlayerScoreDisplay({ name, score, color }: Props) {
    const colorDisplayText = capitalizeFirstLetter(color);
    return (
        <div className="flex min-w-0 flex-row items-center gap-1.5">
            <ChessKingIcon
                className={`size-6 pb-0.5 shrink-0 ${color === 'white' ? 'text-zinc-200' : 'text-zinc-950'}`}
                title="Player color icon"
            />
            <div className="flex min-w-0 items-baseline">
                <span
                    className="flex-1 truncate lg:text-lg text-sm font-semibold text-zinc-100"
                    title={`${colorDisplayText} player: ${name}`}
                >
                    {name}
                </span>
