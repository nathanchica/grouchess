...
import { formatMinutesAndSeconds, formatMsPart, parseTime } from '../../utils/clock';
        ...
        >
            {isActive && (
                <StopwatchIcon
                    title="Stopwatch"
                    role="img"
                    className="inline-block size-5 text-zinc-300/80 ml-4"
                    aria-hidden="true"
                />
            )}
            <span
            ...
            >
                {minAndSecText}
                <span className={`text-base ${!showMsPart && 'invisible'}`} data-testid="clock-milliseconds">
