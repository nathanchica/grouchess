...
const DRAW_STATUS_TO_DISPLAY_TEXT: Partial<Record<GameStatus['status'], string>> = {
    stalemate: 'Stalemate',
...
export function isDrawStatus(status: GameStatus['status']): boolean {
    return DRAW_STATUSES.has(status);
}

export function getDisplayTextForDrawStatus(status: GameStatus['status']): string {
